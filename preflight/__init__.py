...
-----------
    ...
    print(report)           # terminal summary
    report.to_dict()        # machine-readable
    report.to_markdown()    # markdown for model cards

    # Train/test drift check (stretch goal)
...
def check(
    ...
    """
    Run the full preflight checklist on *df*.
    ...
    Returns
    -------
    Report object with .score, .verdict, __str__, .to_dict(), .to_markdown()
    """
...
# ── check_split: train/test distribution drift ───────────────────────────────
...
def check_split(
    ...
    Uses Population Stability Index (PSI) per numeric column.
    PSI < 0.1  → stable  (PASS)
    PSI 0.1–0.2 → minor shift (WARN)
    PSI > 0.2  → major shift  (FAIL)

    Parameters
    ----------
    X_train       : training split (features only, no target)
    X_test        : test split (features only, no target)
    threshold_psi : PSI threshold for FAIL verdict (default 0.2)

    Returns
    -------
    Report summarising per-column drift
    """
    ...
    for col in numeric_cols:
        ...
        if psi is None:
            continue
        record = {"column": col, "psi": round(psi, 4)}
    ...
    # Categorical drift via total variation distance
    ...
    for col in common_cols:
        ...
        if delta >= cfg.split.missingness_fail_delta:
            missingness_fail.append(rec)
        elif delta >= cfg.split.missingness_warn_delta:
            missingness_warn.append(rec)
...
def _compute_categorical_tvd(train_col: pd.Series, test_col: pd.Series) -> Optional[float]:
    """
    Total variation distance between train and test category distributions.
    """
...
def _prepare_runtime_df(df: pd.DataFrame, config: PreflightConfig) -> pd.DataFrame:
    """
    Apply runtime sampling policy for large datasets or explicit fast mode.
    """
